    ...
    case 'M':
      ...
      switch(cmd)
      {
        ...
        case 109: //M109
          if (fromTFT)
          {
            infoCmd.queue[infoCmd.index_r].gcode[cmd_index + 3]='4';  // Avoid send M109 to Marlin SUTAS???
        ...
        case 190: //M190
          if (fromTFT)
          {
            infoCmd.queue[infoCmd.index_r].gcode[cmd_index + 2]='4';   // Avoid send M190 to Marlin SUTAS???
            if (cmd_seen('R'))
            {
              infoCmd.queue[infoCmd.index_r].gcode[cmd_index-1] = 'S';
